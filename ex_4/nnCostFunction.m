...
z{l+1}=a{l}*Theta{l}';
a{l+1}=sigmoid(z{l+1});
    if l+1<L        %to prevent adding bias unit for last layer
        a{l+1} = [ones(size(a{l+1},1), 1) a{l+1}];  
    end
end
...
delta{3}=a{3}-y2;
for l=L-1:-1:2
    delta{l}=(delta{l+1}*(Theta{l}(:,2:end))).*sigmoidGradient(z{2});   
end
...
% Big Delta calculation (with for loop is commented)
%for t=1:m
    for l=1:L-1
        Delta{l}=Delta{l}+((delta{l+1})'*(a{l}));   
    end
%end (t,:)(t,:)% if want for loop , elements address for delta{l+1} and
%a{l} are (t,:) where t is itteration over training samples
   
      
% gradient descent with reg for all columns extept first one
    for l=1:L-1
        for i=1:(size(Delta{l},1))
            for j=1:(size(Delta{l},2))
                if j==1
                   D{l}(i,j)=(1/m)*(Delta{l}(i,j));   %not regularizing the first column of theta
                else
                   D{l}(i,j)=(1/m)*(Delta{l}(i,j)+lambda*Theta{l}(i,j));
                end
            end
        end
    end
